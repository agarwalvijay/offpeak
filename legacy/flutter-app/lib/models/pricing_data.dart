...
class PricingPoint {
  ...
  PricingPoint({required this.dateTime, required this.price});

  factory PricingPoint.fromJson(Map<String, dynamic> json) {
    final rawMillis = json['millisUTC'];
    final millisUtc = rawMillis is int
        ? rawMillis
        : int.parse(rawMillis.toString());
    ...
    return PricingPoint(
      dateTime: DateTime.fromMillisecondsSinceEpoch(millisUtc).toLocal(),
      price: price,
    );
  }

  String get formattedTime => DateFormat('HH:mm').format(dateTime);
  String get formattedDate => DateFormat('MMM d').format(dateTime);
  String get formattedDateTime => DateFormat('MMM d, HH:mm').format(dateTime);
...
enum AlertLevel { low, normal, medium, high }
